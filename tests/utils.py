...
def assert_exercise_response(response_data: Dict[str, Any], expected_name: str, expected_category: str, expected_equipment: str) -> None:
...
def assert_user_response(response_data: Dict[str, Any], expected_username: str, expected_display_name: str = None) -> None:
...
def assert_session_response(response_data: Dict[str, Any], expected_user_id: int, has_details: bool = False) -> None:
...
def assert_template_response(response_data: Dict[str, Any], expected_name: str, expected_user_id: int, has_exercises: bool = False) -> None:
...
def assert_set_response(response_data: Dict[str, Any], expected_weight: float, expected_reps: int, expected_unit: str) -> None:
...
def create_exercise_payload(name: str = "Test Exercise", category: str = "chest", equipment: str = "barbell") -> Dict[str, str]:
...
def create_user_payload(username: str = "testuser", display_name: str = "Test User") -> Dict[str, str]:
...
def create_template_payload(user_id: int, name: str = "Test Template", description: str = None) -> Dict[str, Any]:
...
def assert_datetime_recent(datetime_str: str, max_seconds_ago: int = 10) -> None:
    ...
    assert diff <= max_seconds_ago, f"Datetime is too old: {datetime_str} ({diff} seconds ago)"
...
def assert_pagination_response(response_data: Any, expected_min_items: int = 0) -> None:
    ...
    assert len(response_data) >= expected_min_items, f"Expected at least {expected_min_items} items"
