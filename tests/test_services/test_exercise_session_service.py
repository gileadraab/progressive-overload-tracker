...
class TestGetExerciseSessions:
    ...
    def test_get_all_exercise_sessions_from_session(
        self, db_session: Session, sample_session: SessionModel, sample_exercise: Exercise
    ):
        """Test getting all exercise sessions from a session."""
        # Create sample exercise sessions
        exercise_sessions = [
            ExerciseSession(session_id=sample_session.id, exercise_id=sample_exercise.id),
            ExerciseSession(session_id=sample_session.id, exercise_id=sample_exercise.id),
        ]
    ...
    def test_get_all_exercise_sessions_from_template(
        self, db_session: Session, sample_template: TemplateModel, sample_exercise: Exercise
    ):
        """Test getting all exercise sessions from a template."""
        # Create exercise session for template
        exercise_session = ExerciseSession(template_id=sample_template.id, exercise_id=sample_exercise.id)
    ...
    def test_get_exercise_sessions_with_pagination(
        ...
        for i in range(5):
            es = ExerciseSession(session_id=sample_session.id, exercise_id=sample_exercise.id)
            db_session.add(es)
        db_session.commit()

        # Get first 2
        result = exercise_session_service.get_exercise_sessions(db_session, skip=0, limit=2)
        assert len(result) == 2

        # Get next 2
        result = exercise_session_service.get_exercise_sessions(db_session, skip=2, limit=2)
        assert len(result) == 2

        # Get last one
        result = exercise_session_service.get_exercise_sessions(db_session, skip=4, limit=2)
        assert len(result) == 1
    ...
    def test_get_exercise_sessions_with_nested_data(
        ...
        exercise_session = ExerciseSession(session_id=sample_session.id, exercise_id=sample_exercise.id)
        db_session.add(exercise_session)
        db_session.commit()
...
class TestGetExerciseSession:
    ...
    def test_get_existing_exercise_session(
        ...
        result = exercise_session_service.get_exercise_session(exercise_session.id, db_session)
...
class TestCreateExerciseSession:
    ...
    def test_create_exercise_session_persisted(
        ...
        created_es = exercise_session_service.create_exercise_session(es_data, db_session)
...
class TestUpdateExerciseSession:
    ...
    def test_update_exercise_session_exercise(
        ...
        result = exercise_session_service.update_exercise_session(exercise_session.id, update_data, db_session)
    ...
    def test_update_exercise_session_from_session_to_template(
        self, db_session: Session, sample_session: SessionModel, sample_template: TemplateModel, sample_exercise: Exercise
    ):
        ...
        update_data = ExerciseSessionUpdate(template_id=sample_template.id, session_id=None)
        result = exercise_session_service.update_exercise_session(exercise_session.id, update_data, db_session)

        assert result.template_id == sample_template.id
        assert result.session_id is None

    def test_update_nonexistent_exercise_session(self, db_session: Session, sample_exercise: Exercise):
        ...
        with pytest.raises(HTTPException) as exc_info:
            exercise_session_service.update_exercise_session(999, update_data, db_session)
        assert exc_info.value.status_code == 404
        assert "not found" in exc_info.value.detail.lower()
...
class TestDeleteExerciseSession:
    ...
    def test_delete_exercise_session_returns_none(
        ...
        result = exercise_session_service.delete_exercise_session(exercise_session.id, db_session)
